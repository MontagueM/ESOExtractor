...
def save_mnf_table(file_table, b_is_eso):
    ...
    entries = [(x.FileIndex, x.ID1, x.Index, x.Unk1, x.Size, x.CompressedSize, x.Hash, x.Offset, x.CompressType, x.ArchiveIndex, x.Unk2) for x in file_table]
    c.execute(f'CREATE TABLE IF NOT EXISTS {name} (FileIndex INTEGER, ID1 INTEGER, Indexx INTEGER, Unk1 INTEGER, Size INTEGER, CompressedSize INTEGER, Hash INTEGER, Offset INTEGER, CompressType INTEGER, ArchiveIndex INTEGER, Unk2 INTEGER)')
    c.executemany(f'INSERT INTO {name} (FileIndex, ID1, Indexx, Unk1, Size, CompressedSize, Hash, Offset, CompressType, ArchiveIndex, Unk2) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);',
              entries)
    con.commit()
